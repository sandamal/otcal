...
async function loadAttendanceTable() {
    ...
    try {
        ...
        const attRecords = await FB.getMonthlyAttendance(currentTellerId, monthStr);
        const attMap = new Map(attRecords.map(r => [r.date, r]));
        const team = currentTellerProfile?.team || 'team1';
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            if (roster) {
                ...
                else rosterEntry = roster.tellers?.find(t => t.teller === currentTellerId) || null;
            }
            ...
            rows.push({ dateStr, tellerId: currentTellerId, rosterEntry, isOff, arrival, departure, booth, status, ot, workHours, shift });
        }
...
function renderTable(rows) {
    ...
    rows.forEach(row => {
        ...
        if (shift !== 'OFF' && actualArrival && actualDeparture) {
            ...
            let targetHours = 0;
            if (status === 'Present') targetHours = 8;
            else if (status.includes('Half')) targetHours = 4;
            else if (status.includes('Short')) targetHours = 1.5;
            else targetHours = 0;

            ot = Math.max(0, workHours - targetHours);
        } else if (shift === 'OFF') {
            status = 'OFF';
        ...
        tr.innerHTML = `
            ...
            <td>${row.booth}</td>
            <td class="work-hours-cell">${workHours}</td>
            <td class="ot-cell">${ot.toFixed(2)}</td>
        ...
        inputs.forEach(inp => {
            inp.addEventListener('change', () => handleAttendanceChange(row.tellerId, row.dateStr, tr));
        });
...
async function loadReportTellers() {
    ...
    sel.innerHTML = '';
    if (currentTellerProfile && currentTellerId) {
...
async function generateReport() {
    ...
    if (tellerId !== currentTellerId) return alert("Authorization Error: You can only view your own reports.");
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        else if (status === 'Present') workHours = 8;
    ...
    // OT formula: 180 - (Leave + Off + Short Leaves + Half Days)
    let totalOTHours = 180
        - (stats.leaveHours + (stats.off * 8) + (stats.shorts * 1.5) + (stats.halfs * 4));
    ...
    records.forEach(r => {
        ...
        // Event listener for inline status edit
    ...
    document.getElementById('report-payable-hours').textContent = payableHours.toFixed(2);
    ...
    document.getElementById('report-total-worked-hours').textContent = stats.totalWorkedHours.toFixed(2);
    document.getElementById('report-leave-hours').textContent = stats.leaveHours;
}

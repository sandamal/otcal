    ...
    records.forEach(r => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <select class="status-edit" data-date="${r.dateStr}">
                    ...
                    <option value="OFF (Worked)" ${r.status === 'OFF (Worked)' ? 'selected' : ''}>OFF (Worked)</option>
                </select>
            </td>
        `;
        ...
        timeInputs.forEach(inp => {
            inp.addEventListener('change', async () => {
                ...
                if (newArr && newDep) {
                    const stats = calculateDailyStats(newArr, newDep, shift);
                    newWorkHours = stats.workHours;
                    newStatus = shift === 'OFF' ? 'OFF (Worked)' : stats.status;
                } else if (shift === 'OFF') {
                    newStatus = 'OFF';
                }
                ...
                tr.querySelector('.work-hours-cell').textContent = newWorkHours;
                tr.querySelector('.status-edit').value = newStatus;
        ...
        tr.querySelector('.status-edit').addEventListener('change', async (e) => {
            ...
            let newWorkHours = 0;
            if (newArr && newDep && newStatus !== 'OFF' && newStatus !== 'Absent') {
...
function updateReportSummary() {
    let tableWorkedHours = 0;
    let working = 0, off = 0, lates = 0, shorts = 0, halfs = 0, leaveHours = 0;

    document.querySelectorAll('#report-body tr').forEach(tr => {
        const workHoursCell = tr.querySelector('.work-hours-cell');
        const statusSelect = tr.querySelector('.status-edit');
        if (!workHoursCell || !statusSelect) return;
        ...
        if (status === 'OFF') off++;
        else if (status === 'Absent') leaveHours += 8;
        else {
            working++;
    ...
    let totalOTHours = 180 - (leaveHours + (shorts * 1.5) + (halfs * 4));

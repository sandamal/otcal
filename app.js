...
async function populateAdminTellerSelectors() {
    // Team-admin: show only team users. Super-admin (no team): show all users.
    const adminTeam = currentTellerProfile?.team;
    const allTellers = adminTeam
        ? await FB.getTellersByTeam(adminTeam)
        : await FB.getAllTellersWithIds();
...
function setupAuthForms() {
    ...
    document.getElementById('signup-form').addEventListener('submit', async (e) => {
        ...
        const isAdminMode = document.querySelector('.auth-container').classList.contains('admin-mode');
        const data = {
            name: document.getElementById('signup-name').value,
            nick: document.getElementById('signup-nick').value,
            team: isAdminMode ? (document.getElementById('signup-team').value || '') : document.getElementById('signup-team').value,
...
function setupAdminTrigger() {
    ...
    trigger.addEventListener('click', () => {
        ...
        if (clickCount === 3) {
            ...
            // Toggle Teller ID requirement and Team optionality
            if (container.classList.contains('admin-mode')) {
                tellerIdInput.removeAttribute('required');
                tellerIdInput.value = ''; // Clear it
                // Make team optional — add "All Teams" option for super-admin
                teamInput.removeAttribute('required');
                const allOpt = document.createElement('option');
                allOpt.value = '';
                allOpt.textContent = 'All Teams (Super Admin)';
                teamInput.insertBefore(allOpt, teamInput.firstChild);
                teamInput.value = ''; // Default to All Teams
            } else {
                tellerIdInput.setAttribute('required', '');
                teamInput.setAttribute('required', '');
                // Remove the "All Teams" option
                const firstOpt = teamInput.querySelector('option[value=""]');
                if (firstOpt && firstOpt.textContent.includes('All Teams')) {
                    firstOpt.remove();
                }
                teamInput.value = ''; // Reset to "Select Team"
            }
...
function renderTable(rows) {
    ...
    rows.forEach(row => {
        ...
        let workHours = row.workHours;
        let status = row.status;

        if (actualArrival && actualDeparture) {
            const stats = calculateDailyStats(actualArrival, actualDeparture, shift);
            workHours = stats.workHours;
            // Use the status from row (which is already escalated)
            // but if we have new times, we might need to reflect 'OFF (Worked)'
            if (shift === 'OFF') status = 'OFF (Worked)';
        }
        else if (shift === 'OFF') {
            status = 'OFF';
        }
...
async function handleAttendanceChange(tellerId, dateStr, tr) {
    try {
        ...
        // Refresh table to re-apply monthly escalation
        loadAttendanceTable();
...
function applyMonthlyEscalation(dailyResults) {
    ...
    return dailyResults.map(day => {
        ...
        // 1. Late Escalation (Persistent after 4th Late)
        if (status === "Late") {
            lateCount++;
            if (lateCount >= 5) {
                status = "Short Leave";
        ...
        // 2. Short Leave Escalation (Persistent after 2nd Short Leave)
        // This includes lates converted to Short Leaves
...
async function generateReport() {
    ...
    const escalatedRecords = applyMonthlyEscalation(records);

    // ===== Calculate Summary Statistics after Escalation =====
    ...
    document.getElementById('report-lates').textContent = stats.lates;
    document.getElementById('report-short-leaves').textContent = stats.shorts;
...
function updateReportSummary() {
    ...
    document.getElementById('report-lates').textContent = lates;
    document.getElementById('report-short-leaves').textContent = shorts;
